...
const CustomModalForm = ({selectedID}) => {
  ...
  const centred = {
    ...
    const getData = async (selectedID) => {
        ...
        console.log(`test data====> ${       assetList.assets&&  JSON.stringify(assetList.assets[0].data.cpu_architecture, null, " ")}`)
      };
    ...
    return(
        <>
        {loading === true ? <><div style={{display:'flex', justifyContent:'center'}}><h3>Connecting.... </h3></div><Spinner load = {loading}/></> :
        <div
        style={{
